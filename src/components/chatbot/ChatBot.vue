...
/* ========== 데이터 ========== */
const systemPrompt =
  '당신은 맞춤형 금융상품 비교/추천 플랫폼의 친절하고 전문적인 챗봇입니다. 사용자로부터 입력받은 금융 성향, 소비 패턴, 저축 목표 등의 정보를 바탕으로 적절한 금융 상품(카드, 예금, 적금 등)을 추천하고, 금융 용어와 제도에 대한 이해를 돕습니다. 또한 사용자가 금융 지식이 부족하더라도 쉽게 이해할 수 있도록 쉬운 언어로 설명하며, 퀴즈 및 교육 콘텐츠로 금융 리터러시 향상을 지원합니다.';
...
const fetchDefinitionFromGPT = async (term) => {
  try {
    const res = await axios.post('/api/chatbot', {
      ...
      systemPrompt:
        '당신은 금융 전문가입니다. 쉬운 한국어로, 군더더기 없이 친절하게 설명하세요.',
    });
...
const fetchSimplerDefinitionFromGPT = async (term) => {
  try {
    const res = await axios.post('/api/chatbot', {
      ...
      systemPrompt:
        '당신은 친절한 선생님입니다. 어려운 용어 금지, 짧고 쉬운 문장.',
    });
    const reply = res?.data?.reply?.trim();
...
const sendToGPT = async (text) => {
  ...
  const isFinancialTopic = financialKeywords.some((k) => text.includes(k));
  if (!isFinancialTopic)
    return '⚠️ 죄송합니다. 저는 맞추머니 AI 챗봇으로서 금융 관련 질문만 도와드릴 수 있어요!';
